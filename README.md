...
| Cross-attention over item attributes | Novel    | Training in progress (full-data run started 2026-04-27 18:08; on epoch 20 / valid_score 0.181 as of 2026-04-28 15:32, ~1 hour per epoch, validation gain decelerating ~0.0002/epoch — early stopping (default `stopping_step=10`) expected within 5–15 more epochs). Evaluation auto-fires via the queued dependency-job; results land in `slurm_logs/evaluation_NovelModel_out.txt` and the two plot scripts auto-fill on next render. All upstream artifacts complete: item-attribute parquet, title/brand/color MiniLM embeddings, 32-quantile price bins, model-side loader helpers, `NovelModel(SequentialRecommender)` subclass, slot-toggle ablation infrastructure (see "Ablation runs" above). |
