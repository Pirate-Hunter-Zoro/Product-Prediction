...
scripts/
  ...
  plot_utils.py            Slurm-log JSON parsers shared across plot scripts. `load_eval_log` parses an `evaluate.py` log (single JSON block) by slicing first-`{` to last-`}` and calling `json.loads`; returns `Dict[str, Any]`. `load_pop_log` splits a `pop_baseline.py` log on the literal `"Session Metrics:"` marker (two JSON blocks), runs the same first-`{` to last-`}` slice on each half, and returns `{"global": {...}, "session": {...}}`.
  plot_model_comparison.py IN PROGRESS 2026-04-27. Bar chart over Popularity, GRU4Rec, NARM, NovelModel for the "Overall" (six-locale union) scope. `collect_overall_metrics()` implemented: reads Pop via `load_pop_log` `"global"` mode + each deep model's `"Overall"` key from `load_eval_log`, filters to `METRICS=("mrr@100","recall@100")`, falls back to `math.nan` for any missing log file (NovelModel will be NaN until full training finishes). `render_chart()` and `__main__` argparse glue still pending.
